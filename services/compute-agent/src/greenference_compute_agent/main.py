"""FastAPI application entry point for the greenference compute agent."""
...
from greenference_persistence import database_ready
...
from greenference_compute_agent.transport.routes import router
...
async def _compute_worker_loop() -> None:
...
async def lifespan(_: FastAPI):
    ...
    if settings.enable_background_workers:
        task = asyncio.create_task(_compute_worker_loop())
    try:
        yield
    finally:
        if task is not None:
...
app = FastAPI(
    title="Greenference Compute Agent",
    version="0.1.0",
    description="Lium + Targon style compute provider daemon for POD and VM workloads.",
    lifespan=lifespan,
)
